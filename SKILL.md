...
artifact 目录不强制固定顶层文件名。顶层可以按需要创建少量 scope、manifest、index、source locator、cross-component summary、HTML final report 等轻量文件；Markdown 只作为可选 companion。`glossary.html` 是 mandatory artifact，必须在开始生成 HTML 详查前创建并持续更新。顶层文件只做导航、状态表、跨组件裁决、可视化浏览和最终汇总，不是详查正文。不要把所有内容压缩进这些单个文档；如果只有顶层汇总/HTML final report 而没有 `components/` 和 `entries/` 下的逐项详查，本轮调研视为未完成。
...
`glossary.html` 是本 skill 的强制 artifact。只要是缩写、存储系统术语、项目特有单词、项目特有 phrase、组件名、协议名、指标名、硬件/内核/I/O 术语、domain-map 术语或容易被误解的英文原词，都必须进入术语表。

术语表规则：
- 默认路径是 artifact 工作区根目录的 `glossary.html`；可选 companion 是 `glossary.md`，只用于 diff/search。
- 可以从 `assets/html/storage-glossary-template.html` 复制模板后填充。
- 每个 term 使用稳定 anchor：`#term-<slug>`。slug 使用小写 ASCII、数字和 hyphen；缩写保留小写，如 `#term-rdma`、`#term-fdb`、`#term-write-amplification`。
- 每个 entry 至少包含：`Term`、`Aliases / Abbreviations`、`Type`、`Chinese Explanation`、`Project Meaning`、`Why It Matters`、`Source / Evidence`、`Related Terms`、`Status`。
- `Type` 可取：`abbreviation`、`storage-term`、`project-term`、`component`、`metric`、`protocol`、`hardware`、`kernel`、`domain-map`、`external-system`。
- `Status` 可取：`confirmed` / `inferred` / `needs-code-proof` / `ambiguous`。

链接规则：
- 之后生成的所有 HTML 中，正文、标题、表格、卡片、callout、图标签里出现的术语都必须链接到 `glossary.html#term-...`。
- 代码块、原始源码片段、文件路径、函数名的纯代码 token 可以不改写；但其旁边的解释文字中出现同一术语时必须链接。
- 如果写 HTML 时遇到尚未在术语表中的术语，先新增 `glossary.html` entry，再在当前 HTML 中链接它。
- 每次完成一个 HTML 文件前，做一次术语扫描：检查缩写、存储术语、项目 noun phrase 是否都已入表并链接；遗漏项必须补齐，不能留给最终报告再处理。
- Mermaid label 中出现术语时，优先使用 HTML label 或在图后 legend 中链接对应术语；不要因为 Mermaid 难链接就跳过术语表维护。
...
- 每篇 HTML 详查和最终报告都必须链接到 `glossary.html`，并把缩写、存储术语和项目 phrase 链接到对应 term anchor。

## 深度预算和完整性门槛

默认不要追求短。这个 skill 的产物应当偏长、可审阅、可跳转、可复用；如果材料太少，应明确写成缺口，而不是用短摘要填满页面。

每篇 `components/**.html` 或 `entries/**.html` 详查至少满足：
- 使用完整 HTML 页面，不要只生成 card 片段；推荐从 `assets/html/storage-deep-dive-template.html` 复制后填充。
- 至少 8 个实质 section：overview、code map、code walkthrough、data/state flow、control/concurrency flow、cross-component interactions、project document analysis、external context、cannot claim、gaps、next probes 中选择并保留适用项。
- 至少列出 10 个具体代码锚点；如果目标模块小于 10 个锚点，必须说明“模块很小”并列出已搜到的全部锚点和搜索方法。
- 至少包含 3 段较完整的代码逻辑解释，每段解释一个真实路径、状态变化、错误路径、并发边界或持久化语义；不要用一句话代替分析。
- 至少包含 2 个短源码片段或函数/类型签名，并分别说明“这证明什么 / 不能证明什么”。
- 至少包含 1 个 Mermaid 图；涉及读写路径、恢复路径、后台任务或跨 3 个以上组件时至少 2 个图。
- 至少包含 1 个表格：code anchor table、state ownership table、source locator table、risk table 或 probe table。
- 至少包含 1 个 worked example：一个具体请求、写入、读取、恢复、重平衡、后台任务、指标变化或故障传播场景。
- 至少包含 5 个术语表链接；如果术语不足 5 个，必须说明为什么。

## 组件代码分析视角

详查必须以组件代码为中心。下面这些不是“覆盖清单”，而是分析组件代码的视角。不要为每个视角写一行浅摘要；要选择与该组件设计最相关的视角，围绕具体代码路径、数据结构、状态变化、并发边界和跨组件调用深入解释“代码是如何写的、为什么这样设计、这样设计的代价是什么”。

每篇与 I/O、性能、持久化、分布式路径、硬件路径或后台任务相关的详查，都必须先画出组件的主代码路径，然后用以下视角审视这些路径；如果某视角不适用，必须写 `Not applicable` 并说明是基于哪些代码/文档判断的，不能沉默跳过。

- `Supported I/O Sizes`：支持的最小/最大/默认 I/O size，alignment，block/chunk/object/page/extent 粒度，小 I/O 与大 I/O 是否走不同路径，split/merge/fragmentation 逻辑在哪里。
- `Hot Path / Cold Path`：前台请求、后台维护、初始化、recovery、rebalance、snapshot、scrub、compaction、migration 等路径如何区分；冷路径是否会影响热路径。
- `Fast Path / Slow Path`：cache hit、metadata hit、本地路径、无锁路径、batch 路径等 fast path；miss、retry、conflict、fallback、degraded、repair、remote path 等 slow path。
- `Memory Data Structures`：核心内存结构、索引、cache、queue、buffer、arena/slab、refcount、ownership、lifetime、eviction、memory pressure 和一致性语义。
- `Software Overhead`：线程切换、协程调度、锁竞争、队列排队、事务冲突、序列化/反序列化、copy、checksum、压缩、加密、灾难恢复路径的额外成本。
- `Threading Model`：多线程、协程、同步/异步 I/O、reactor、worker pool、run-to-completion、completion routing、CPU affinity、NUMA 和 backpressure。
- `Storage Stack Layers`：逻辑资源池、volume/object/chunk/extent、RAID/EC、远程镜像复制、分层缓存、QoS、多链路、多路径、cache tier、placement 与 mapping 的模块堆叠。
- `High-Performance SSD Path`：NVMe、SPDK、io_uring、ZNS、append/write pointer、zone reset、polling、QD、doorbell、DMA、zero-copy、flush/FUA/barrier 等优化和限制。
- `High-Performance Network Path`：RDMA、RPC、zero-copy、registered memory、completion queue、send/recv queue、connection management、retry、congestion、multi-rail、多链路和 fallback。
- `Distributed Consistency and HA`：replication、EC、quorum、consensus、lease、epoch、membership、split-brain、防脑裂、linearizability/causal/eventual consistency、failover、recovery、degraded mode。

这些视角必须落到代码：每个适用视角至少要解释相关文件/函数/类型/配置如何共同实现该设计，并说明这个实现对 state/data flow、control/concurrency flow、cross-component interactions 的影响。没有代码证据时标为代码缺口。测试、benchmark 或外部资料只能补充方法学，不能替代这些视角的源码分析。

推荐写法：
- 先写组件主线：入口函数、核心类型、核心状态、关键调用链。
- 再写视角分析：从上面的视角中选择最能解释该组件设计的 3-6 个深入展开；其余视角可以简要说明“不适用/待证据”。
- 每个深入视角都必须包含：相关代码锚点、设计解释、对状态/并发/交互的影响、风险或 cannot-claim。
- 任何矩阵或表格只能做导航，不能代替正文分析。

最终 HTML report 不是短摘要页。它至少应包含 8-12 张有内容的卡片，覆盖系统架构、关键组件、关键跨组件路径、最高风险 architecture entries、最高风险 metrics/universal metrics、mandatory artifacts 和 next probes。每张卡都要有解释、短代码或代码锚点、图/表、风险/cannot-claim 和详查链接。

HTML 代码本身也不能过短：不要生成只有几段 `<section>` 的临时页面。每个主 HTML artifact 都应该包含稳定页面骨架、导航/TOC、状态 badge、术语链接、源码区域、图区域、表格区域和回链。缺少这些结构时，应继续扩展 HTML，而不是交付。
...
- 组件/entry 详查页优先从 `assets/html/storage-deep-dive-template.html` 复制后填充；最终报告从 `assets/html/storage-investigation-report-template.html` 复制后填充。模板只是起点，必须按实际项目删改和扩展。
...
- `Investigation Map`：artifact manifest、`glossary.html`、组件详查、entry 详查、source locator、final report、可选 Markdown companion 的链接。
...
- `Storage-Specific Axes Matrix`：汇总 supported I/O sizes、hot/cold path、fast/slow path、memory data structures、software overhead、threading model、storage stack layers、SSD、network/RDMA、consistency/HA 在各组件/entry 中的覆盖情况。
...
- `Glossary Links`：本 card 中出现的核心术语链接，至少列出新增或关键术语；正文中也必须内联链接。
...
- `assets/html/storage-glossary-template.html`：生成 mandatory `glossary.html` 时使用的静态模板。开始生成 HTML 详查前读取或复制。
- `assets/html/storage-deep-dive-template.html`：生成组件/entry 详查页时使用的长格式静态模板。默认需要组件或 entry HTML 详查时读取或复制。
